...
function GestionDeProductos() {
    ...
    const handleOpenModalEdit = () => { setIsViewOpenEdit(true); }
    const handleCloseModalEdit = () => { setIsViewOpenEdit(false); }

    const handleOpenModalBalances = () => { setIsModalOpenBalances(true); }
    const handleCloseModalBalances = () => { setIsModalOpenBalances(false); }

    const handleOpenModalInventoryAdjustment = () => { setIsModalOpenInventoryAdjustment(true); }
    const handleCloseModalInventoryAdjustment = () => { setIsModalOpenInventoryAdjustment(false); }
                ...
                <div className="w-screen h-fit m-5 p-3 border-2 border-rose-500 rounded-lg flex justify-center flex-wrap shadow-md shadow-slate-400 drop-shadow-xl">
                    ...
                    <div className="text-center w-full">
                        ...
                        <div className="flex flex-col gap-3 justify-start items-start my-3 ml-14">
                            ...
                            <input type="text" className="placeholder-orange-950 w-2/5 text-start block p-2 pl-10 rounded-lg bg-gray-100 border border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:placeholder text-sm" placeholder="Buscar Producto por código de barra ó nombre" />
                        </div>
                    </div>
                    <table className="w-11/12 my-5 table-fixed text-start">
                        ...
                        <tbody className="">
                            <tr className="text-center border-b odd:bg-white even:bg-slate-300">
                                ...
                                <td className=""><button onClick={handleOpenModalBalances} className="flex flex-col items-center mx-4 bg-orange-500 my-3 border-indigo-800 rounded-xl px-4 py-1 text-center h-auto w-auto align-middle shadow-md shadow-emerald-600 border-opacity-60 border-2 hover:border-opacity-0 hover:brightness-90 font-semibold"><i>2</i>Saldos</button></td>
                                <ProductBalances isOpen={isModalOpenBalances} onClose={handleCloseModalBalances} />
                                ...
                                <td className=""><button onClick={handleOpenModalInventoryAdjustment} className="flex flex-col items-center mx-4 bg-indigo-500 text-stone-200 my-3 border-indigo-800 rounded-xl px-4 py-1 text-center h-auto w-auto align-middle shadow-md shadow-emerald-600 border-opacity-60 border-2 hover:border-opacity-0 hover:brightness-90 font-semibold"><i>2</i>Ajuste</button></td>
                                <InventoryAdjustment isOpen={isModalOpenInventoryAdjustment} onClose={handleCloseModalInventoryAdjustment} />
