...
const EmpleadosPermisos: React.FC = () => {
  ...
  const validateStep1 = () => {
    if (!formData.num_doc.trim() || !formData.password.trim()) {
      setError("El número de documento y la contraseña son obligatorios.");
      return false;
    }
      ...
      <div className="max-w-7xl mx-auto space-y-4">
        ...
        <div className="bg-white shadow-xl rounded-lg border border-gray-200 overflow-hidden">
          <div className="bg-gray-50 border-b border-gray-200 flex p-2 gap-2">
            <button
              onClick={() => { setActiveTab("empleados"); setView("list"); }}
            ...
            <button
              onClick={() => { setActiveTab("roles"); setView("list"); }}
          ...
          <div className="p-4 md:p-6">
            {activeTab === "empleados" && view === "list" && (
              <div className="animate-in fade-in duration-500">
                <div className="flex justify-center mb-6">
                  <button
                    onClick={() => { setView("add"); setStep(1); setError(null); setSuccessMessage(null); }}
                ...
                <div className="flex flex-wrap justify-between items-center mb-4 gap-4">
                  ...
                  <div className="relative">
                    ...
                    <input type="text" placeholder="Buscar..." className="pl-10 pr-4 py-1.5 border border-gray-300 rounded-lg text-sm focus:ring-2 focus:ring-blue-500 outline-none w-64" />
                  </div>
                </div>
                ...
                <div className="overflow-x-auto border border-gray-200 rounded-xl">
                  <table className="w-full text-left text-sm border-collapse bg-white">
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {loading ? (
                         <tr><td colSpan={9} className="text-center p-4">Cargando usuarios...</td></tr>
                      ) : users.map((user) => (
                        <tr key={user.id} className="hover:bg-blue-50/50 transition-colors">
                          <td className="p-3 border-r text-gray-500">{user.tipo_doc}</td>
                          <td className="p-3 border-r font-mono">{user.num_doc}</td>
                          <td className="p-3 border-r font-bold">{user.username}</td>
                          <td className="p-3 border-r font-bold uppercase">{user.full_name}</td>
                          <td className="p-3 border-r text-xs text-gray-500 italic">{user.email || 'N/A'}</td>
                          <td className="p-3 border-r">{user.phone || 'N/A'}</td>
                          <td className="p-3 border-r text-center">
                            <button onClick={() => handleEdit(user)} className="p-1.5 bg-amber-500 text-white rounded hover:bg-amber-600 shadow-sm">
                              <Edit size={14} />
                            </button>
                          </td>
                          <td className="p-3 border-r text-center"><ToggleSwitch checked={user.is_active} /></td>
                          <td className="p-3 text-center">
                            <div className="flex flex-col items-center gap-1">
                              <span className="text-[10px] font-black bg-blue-100 text-blue-700 px-2 py-0.5 rounded uppercase">{user.role}</span>
                            </div>
                          </td>
                        </tr>
                      ))}
                ...
                <div className="space-y-6">
                  {step === 1 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-gray-50 rounded-xl border border-gray-100">
                      <InputField label="Tipo Documento" name="tipo_doc" type="select" options={['CC', 'NIT', 'CE', 'PAS']} value={formData.tipo_doc} onChange={handleChange} required />
                      <InputField label="Número de Documento" name="num_doc" type="text" value={formData.num_doc} onChange={handleChange} required />
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-gray-50 rounded-xl border border-gray-100 animate-in fade-in">
                      ...
                      <InputField label="Rol" name="role" type="select" options={rolesOptions} value={formData.role} onChange={handleChange} required />
                    </div>
                  )}
                  ...
                  <div className="flex justify-center gap-4 pt-4">
                    {step === 1 ? (
                      <>
                        <button onClick={() => setView("list")} className="flex items-center gap-2 px-6 py-2 bg-gray-200 text-gray-700 rounded-lg font-bold hover:bg-gray-300 transition">
                          <ArrowLeft size={18} /> Regresar
                        </button>
                        <button onClick={() => { if (validateStep1()) setStep(2); }} className="flex items-center gap-2 px-6 py-2 bg-blue-600 text-white rounded-lg font-bold hover:bg-blue-700 transition shadow-md">
                          Siguiente <ArrowRight size={18} />
                        </button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => setStep(1)} className="flex items-center gap-2 px-6 py-2 bg-gray-200 text-gray-700 rounded-lg font-bold hover:bg-gray-300 transition">
                          <ArrowLeft size={18} /> Atrás
                        </button>
                        <button onClick={handleSubmitUser} className="flex items-center gap-2 px-6 py-2 bg-green-600 text-white rounded-lg font-bold hover:bg-green-700 transition shadow-md" disabled={loading}>
            ...
            {activeTab === "empleados" && view === "edit" && selectedUser && (
              <div className="max-w-2xl mx-auto animate-in zoom-in-95 duration-300">
                <p className="text-center font-bold text-gray-600 mb-8 border-b pb-2">Actualizar información de {selectedUser.full_name}</p>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 p-6 rounded-xl border border-gray-200">
                  <InputField label="Tipo Documento" name="tipo_doc" type="text" value={selectedUser.tipo_doc} onChange={()=>{}} disabled />
                  <InputField label="Número de Documento" name="num_doc" type="text" value={selectedUser.num_doc} onChange={()=>{}} disabled />
                  <InputField label="Nombre" name="name" type="text" value={selectedUser.name} onChange={()=>{}} required />
                  <InputField label="Apellido" name="last_name" type="text" value={selectedUser.last_name || ''} onChange={()=>{}} />
                  <InputField label="Email" name="email" type="text" value={selectedUser.email || ''} onChange={()=>{}} />
                  <InputField label="Teléfono" name="phone" type="text" value={selectedUser.phone || ''} onChange={()=>{}} />
                  <div className="md:col-span-2">
                    <InputField label="Dirección" name="address" type="text" value={selectedUser.address || ''} onChange={()=>{}} />
                  </div>
                   <InputField label="Rol" name="role" type="text" value={selectedUser.role} onChange={()=>{}} disabled />
                </div>
                ...
                <div className="border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                   <table className="w-full text-left text-sm bg-white">
                    <thead className="bg-gray-100 text-gray-700 font-bold uppercase text-[11px]">
                      <tr><th className="p-4 border-r">Rol Nombre</th><th className="p-4 text-center">Editar</th></tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200">
                      {rolesOptions.map((rol, index) => (
                        <tr key={index} className="hover:bg-gray-50 transition-colors">
                          <td className="p-4 font-bold border-r text-blue-800 uppercase">{rol}</td>
                          <td className="p-4 text-center">
                            <button className="p-1.5 bg-amber-500 text-white rounded hover:bg-amber-600 transition shadow-sm"><Edit size={14} /></button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
...
const InputField = ({ label, name, type, options, value, onChange, disabled, required }: InputFieldProps) => (
  <div className="flex flex-col group">
    ...
    {type === "select" ? (
      <select id={name} name={name} value={value} onChange={onChange} disabled={disabled} className="bg-transparent border-b-2 border-gray-300 focus:border-blue-600 outline-none py-1.5 text-sm font-semibold transition-all">
        <option value="">Seleccione...</option>
        {options?.map((opt: string) => <option key={opt} value={opt}>{opt.toUpperCase()}</option>)}
      </select>
    ) : (
      <input id={name} name={name} type={type} placeholder={label} value={value} onChange={onChange} disabled={disabled} className={`bg-transparent border-b-2 border-gray-300 focus:border-blue-600 outline-none py-1.5 text-sm font-semibold transition-all ${disabled ? "opacity-50 cursor-not-allowed italic" : ""}`} />
    )}
  </div>
);
