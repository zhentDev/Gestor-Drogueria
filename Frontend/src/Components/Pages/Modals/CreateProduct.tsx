...
const CreateProduct: React.FC<CreateProductProps> = ({ isOpen, onClose }) => {
  ...
  const handleRequiresFormulaChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    setRequiresFormula(event.target.value);
  };

  const handleManagesInventoryChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    setManagesInventory(event.target.value);
  };

  const handleManagesExpirationChange = (
    event: React.ChangeEvent<HTMLInputElement>
  ) => {
    setManagesExpiration(event.target.value);
  };

  const [useIntermedio, setUseIntermedio] = useState(false);
  const [useMinimo, setUseMinimo] = useState(false);
      ...
      {isMosdalOpenHelp_ ? (
        ...
        >
          <div className="rounded-xl bg-white flex flex-col w-2/4 h-2/5 justify-center items-center gap-4">
            <i></i>
            <p className="w-3/4">
              Un producto puede tener asociados varios códigos de barra para
              después ser buscado, ejemplo: "Vitamina C" que se creó con varias
              presentaciones (Unidad, sobre y caja) los diferentes códigos de
              barras se podrán adicionar. Utilizar la pistola o digitar el nuevo
              código asociado y presione el boton, se verán reflejados en la
              tabla inferior.
            </p>
            <button
              className="mx-1 grid col-span-1 h-full p-2 bg-slate-300 border-indigo-800 rounded-xl px-4 py-1 text-center w-fit align-middle items-center shadow-md shadow-emerald-600 border-opacity-60 border-2 hover:border-opacity-0 hover:brightness-90 font-semibold my-8"
      ...
      <div className="w-full h-full m-5 p-3 border-2 border-rose-500 rounded-lg flex flex-col items-center justify-center flex-wrap shadow-md shadow-slate-400 drop-shadow-xl gap-6">
        ...
        <div className="bg-white rounded-lg p-4 w-5/12 flex flex-col items-center max-h-[80vh] overflow-y-auto gap-2 border-2 border-slate-400">
          ...
          <button
            ...
            className="mx-1 col-span-1 h-full p-2 bg-yellow-400 border-indigo-800 rounded-xl px-4 py-1 text-center w-fit align-middle items-center shadow-md shadow-emerald-600 border-opacity-60 border-2 hover:border-opacity-0 hover:brightness-90 font-semibold my-2 inline-flex gap-2"
          >
            <i>?</i>Ayuda
          </button>
          <div className="flex w-full justify-center items-center gap-2 mb-6">
            <label className="w-full">
              <span className="w-full mb-3 text-center">
                Escanea o digita el código de barras
              </span>
              <input
                type="text"
          ...
          <table className="w-11/12">
            <thead className="font-bold bg-slate-600 p-1 px-3 text-start text-slate-100">
              <tr className="flex justify-between border-t-2">
                <td className="uppercase pl-5 text-start w-full" colSpan={2}>
                  CÓDIGO
                </td>
                <td className="uppercase pl-5 text-start w-full" colSpan={2}>
                  ELIMINAR
        ...
        <div className="grid grid-cols-12 w-9/12 gap-4 border-b-2 border-gray-300 pb-4">
          <div className="grid col-span-8 gap-2">
            ...
            <input
              className="w-full border-b-2 border-black mb-4 pb-4 focus:outline-none"
              type="text"
          ...
          <div className="grid col-span-4 gap-2">
            ...
            <input
              className="w-full border-b-2 border-black mb-4 pb-6 focus:outline-none"
              type="text"
              placeholder="Ingrese el registro invima"
            />
          </div>
          <div className="grid col-span-4 gap-2">
            <label className="w-full" htmlFor="name">
          ...
          <div className="grid col-span-4 gap-2">
            ...
            <input
              className="w-full border-b-2 border-black mb-4 pb-4 focus:outline-none"
              type="text"
              placeholder="Ingrese el sustituto (opcional)"
            />
          </div>
          <div className="grid col-span-4 gap-2">
            <label className="w-full" htmlFor="name">
          ...
          <div className="grid col-span-12 grid-cols-12">
            <div className="grid grid-cols-2 col-span-4 text-center border-2 border-slate-300 items-center justify-center p-2">
              <span className="grid col-span-2">
                - ¿Este producto se requiere fórmula medica?
              </span>
              <span className="h-10 items-center text-center flex justify-center">
                <label
                  className="m-2 py-2 text-2xl uppercase"
                  htmlFor="requiresFormula_si"
                >
                  si
                </label>
                <input
                  className="m-2 py-2 size-5 focus:outline-none h-8 pb-2"
                  type="radio"
                  id="requiresFormula_si"
                  name="requiresFormula"
                  value="si"
                  checked={requiresFormula === "si"}
                  onChange={handleRequiresFormulaChange}
                />
              </span>
              <span className="h-10 items-center text-center flex justify-center">
                <label
                  className="m-2 py-2 text-2xl uppercase"
                  htmlFor="requiresFormula_no"
                >
                  no
                </label>
                <input
                  className="m-2 py-2 size-5 focus:outline-none h-8 pb-2"
                  type="radio"
                  id="requiresFormula_no"
                  name="requiresFormula"
                  value="no"
                  checked={requiresFormula === "no"}
                  onChange={handleRequiresFormulaChange}
                />
              </span>
            </div>
            <div className="grid grid-cols-2 col-span-4 text-center border-2 border-slate-300 items-center justify-center p-2">
              ...
              <span className="h-10 items-center text-center flex justify-center">
                ...
                <input
                  className="m-2 py-2 size-5 focus:outline-none h-8 pb-2"
                  type="radio"
                  id="managesInventory_si"
                  name="managesInventory"
                  value="si"
                  checked={managesInventory === "si"}
                  onChange={handleManagesInventoryChange}
                />
              </span>
              <span className="h-10 items-center text-center flex justify-center">
                <label
                  className="m-2 py-2 text-2xl uppercase"
                  htmlFor="managesInventory_no"
                >
                  no
                </label>
                <input
                  className="m-2 py-2 size-5 focus:outline-none h-8 pb-2"
                  type="radio"
                  id="managesInventory_no"
                  name="managesInventory"
                  value="no"
                  checked={managesInventory === "no"}
                  onChange={handleManagesInventoryChange}
                />
              </span>
            </div>
            <div className="grid grid-cols-2 col-span-4 text-center border-2 border-slate-300 items-center justify-center p-2">
              ...
              <span className="h-10 items-center text-center flex justify-center">
                ...
                <input
                  className="m-2 py-2 size-5 focus:outline-none h-8 pb-2"
                  type="radio"
                  id="managesExpiration_si"
                  name="managesExpiration"
                  value="si"
                  checked={managesExpiration === "si"}
                  onChange={handleManagesExpirationChange}
                />
              </span>
              <span className="h-10 items-center text-center flex justify-center">
                <label
                  className="m-2 py-2 text-2xl +"
                  htmlFor="managesExpiration_no"
                >
                  no
                </label>
                <input
                  className="m-2 py-2 size-5 focus:outline-none h-8 pb-2"
                  type="radio"
                  id="managesExpiration_no"
                  name="managesExpiration"
                  value="no"
                  checked={managesExpiration === "no"}
                  onChange={handleManagesExpirationChange}
                />
              </span>
            </div>
            <div className="grid col-start-4 col-span-6 grid-cols-8 items-center justify-center p-2 w-full">
              <span className="w-full grid col-span-7 text-center">
                ...
                <SingleSelect
                  options={[]}
                  name="Ubicación"
                  placeholder="Selecciona la úbicación del punto de venta..."
                  onChange={() => {}}
                />
              </span>
              <button className="mx-1 grid col-span-1 h-full p-2 bg-slate-300 border-indigo-800 rounded-xl px-4 py-1 text-center w-fit align-middle items-center shadow-md shadow-emerald-600 border-opacity-60 border-2 hover:border-opacity-0 hover:brightness-90 font-semibold my-2">
                +
              </button>
            </div>
          </div>
        ...
        <div className="grid grid-cols-12 w-9/12 gap-4 border-b-2 border-gray-300 pb-4">
          <label className="col-span-4 col-start-5" htmlFor="">
            Precio medio compra
          </label>
          <input
            className="col-span-4 col-start-5 border-b-2 border-black h-8 pb-5 focus:outline-none"
            disabled
            type="text"
            placeholder="Precio medio compra"
            title="Precio medio compra"
          />
          <label className="col-span-4 col-start-5" htmlFor="">
          ...
          <div className="border-2 border-slate-400 rounded-lg col-span-12 p-4 flex justify-center flex-col gap-4">
            ...
            <label id="check">
              <input
                ...
                name=""
                id=""
                onClick={() => setUseMinimo(!useMinimo)}
              />
